= Mass Effect 2: adjust Miranda's skin tone
...
. Download the required TexMod file from here
...
[start=6]
. Extract _CoalescedrealCRC_ to the same directory where your _Coalesced.ini_ is located, and run it. It won't take long until another _Coalesced.ini_ shows up with the name Fix_Coalesced.ini.
